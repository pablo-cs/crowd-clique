...
def err():
    return render_template(
        'err.html',
        subtitle='Oh no!',
        text='The username and/or password entered is not correct. '
...
def event_comments():
    ...
    form = CommentForm()
    attendees = Attendance.query.filter_by(event_id=event_id).all()
    in_db = bool(Attendance.query.filter_by(
        event_id=event_id,
...
def remove_attendee():
    ...
    user_name = session.get('user_name')
    event_id = request.form.get('event_id')
    attendee = Attendance.query.filter_by(
            user_name=user_name,
            event_id=event_id).first()
    if attendee:
